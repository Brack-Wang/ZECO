...
val_interval = 25
ddpm_n_epochs = 400
ddpm_val_interval= 25


controlnet_n_epochs = 400
...
if checkpoint_files:
    # Extract the epoch number from each checkpoint filename and find the one with the largest epoch
...
for epoch in range(start_epoch, ddpm_n_epochs):
    ...
    for step, batch in progress_bar:
        ...
        masks = batch["label"].to(device)
        images_cond = torch.cat([images, masks], dim=1).to(device)
        images_cond = images_cond.mean(dim=1, keepdim=True)
        ...
        with autocast(enabled=True):
            ...
            noise_pred = inferer(
                inputs=images_cond, autoencoder_model=autoencoder, diffusion_model=unet, noise=noise, timesteps=timesteps
            )
